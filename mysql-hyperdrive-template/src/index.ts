...
async function handleApiRequest(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === '/api/organizations' && request.method === 'GET') {
      const [rows] = await connection.query('SELECT * FROM organizations ORDER BY id');
    ...
    if (url.pathname === '/api/users' && request.method === 'GET') {
      let query = 'SELECT users.*, organizations.name as organization_name FROM users LEFT JOIN organizations ON users.organization_id = organizations.id';
      const params = [];
    ...
    if (url.pathname.startsWith('/api/users/') && request.method === 'PUT') {
      ...
      const orgId = body.organization_id !== undefined ? 
        (body.organization_id ? Number(body.organization_id) : null) : 
        undefined;
