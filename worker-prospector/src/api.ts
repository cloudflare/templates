...
h.get('/notifiers', async (c) => {
  ...
  const html = notifiers.map(notifier => `
    <tr>
...
h.post("/notifiers", async c => {
  try {
    ...
    await c.env.DB.prepare("insert into notifiers (keyword, email) values (?, ?)").bind(keyword, email).run()
    return c.redirect('/')
  } catch (err) {
    c.status(500)
...
h.get('/urls', async (c) => {
  ...
  const html = notifiers.map(({ url, last_checked }) => `
    <tr>
      <td>${url}</td>
