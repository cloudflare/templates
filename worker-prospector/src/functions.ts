...
export async function xmlToURLs(
  { url, sitemapId, authToken }: { url: string, sitemapId?: number, authToken: string }
  ...
  if (data.sitemapindex) {
    ...
    const sitemaps: Array<string> = data.sitemapindex.sitemap.map((site: any) => site.loc as string)
    return {
...
export async function createSitemaps(sitemapUrl: string, db: D1Database, authToken: string) {
  ...
  try {
    await Promise.all(response.sitemaps.map(sitemap => {
...
export const updateStoredUrls = async (
  db: D1Database,
  authToken: string
) => {
  ...
  const sitemapResponses: Array<SitemapXMLResponse> = await Promise.all(sitemaps!.map(async (sitemap: DBSitemap) => {
    return await xmlToURLs({ url: sitemap.url, sitemapId: sitemap.id, authToken }) as SitemapXMLResponse
  }));

  console.log("Creating URLs in database")

  const preparedStatements =
    sitemapResponses.map(sitemapResponse => sitemapResponse.urls.map(url => createUrl(db, url)))
      .flat()

  return db.batch(preparedStatements)
}

export const addUrlsToQueue = async (
  urls: Array<DBUrl>,
  queue: Queue
) => {
...
export const handleQueuedUrl = async (
  url: DBUrl,
  db: D1Database,
) => {
  ...
  for (const notifier of notifiers) {
    if (text.includes(notifier.keyword)) {
      ...
      const existingNotifierMatch = notifierMatches.find(
        notifierMatch => notifierMatch.notifier_id === notifier.id && notifierMatch.url_id === url.id
      )
...
export const scheduled = async (
  { authToken, db, queue, sitemapUrl }: { authToken: string, db: D1Database, queue: Queue, sitemapUrl: string }
) => {
