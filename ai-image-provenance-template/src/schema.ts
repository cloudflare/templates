import { z } from "zod";
import type { ProvenanceCheck } from "./c2pa";
...
export type ProvenanceResult = {
	...
	timing_ms: { fetch: number; provenance: number | null; total: number };
};
...
export type JobResponse =
	| { job_id: string; status: "queued" | "processing" | "retrying" }
	| { job_id: string; status: "complete"; result: ProvenanceResult }
