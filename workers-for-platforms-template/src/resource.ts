...
export async function PutScriptWithAssetsInDispatchNamespace(
  ...
  try {
    ...
    if (!sessionData.success || !sessionData.result) {
      const errorMsg = sessionData.errors?.[0]?.message || 'Failed to create upload session';
    ...
    if (buckets && buckets.length > 0) {
      ...
      for (let i = 0; i < buckets.length; i++) {
        ...
        if (!uploadResponse.ok || !uploadData.success) {
          const errorMsg = uploadData.errors?.[0]?.message || 'Asset upload failed';
          return { success: false, error: errorMsg };
        }
...
`;
    ...
    formData.append('metadata', new File([JSON.stringify(metadata)], 'metadata.json', { type: 'application/json' }));
    formData.append(scriptFileName, new File([workerCode], scriptFileName, { type: 'application/javascript+module' }));
    ...
    const deployData = await deployResponse.json() as { success: boolean; errors?: Array<{ message: string }> };
    
    if (!deployResponse.ok || !deployData.success) {
      const errorMsg = deployData.errors?.[0]?.message || 'Failed to deploy worker';
      return { success: false, error: errorMsg };
    }
