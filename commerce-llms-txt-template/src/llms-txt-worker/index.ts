...
function getMerchantConfig(env: Env) {
  return {
    ...
    description: env.MERCHANT_DESCRIPTION || "Product catalog powered by Commerce llms.txt",
...
async function getEnrichedCatalog(env: Env, ctx?: ExecutionContext): Promise<EnrichedProduct[]> {
  ...
  if (env.SHOPIFY_STORE_DOMAIN) {
    try {
      catalog = await fetchShopifyCatalog(env.SHOPIFY_STORE_DOMAIN, env.SHOPIFY_STORE_PASSWORD);
      console.log(`[Catalog] Fetched ${catalog.length} products from Shopify (${env.SHOPIFY_STORE_DOMAIN})`);
    } catch (err) {
      console.log(`[Catalog] Shopify fetch failed, using sample catalog: ${err}`);
  ...
  if (env.AI) {
    ...
    console.log(`[Enrichment] No AI binding available (local dev), using fallback enrichment`);
    enriched = catalog.map(fallbackEnrichment);
  }
...
function generateLlmsTxt(products: EnrichedProduct[], full: boolean, config: MerchantConfig): string {
...
app.get("/api", (c) => {
  ...
  return c.json({
    ...
    endpoints: {
      "GET /llms.txt": "Agent-optimized product catalog (llms.txt format, concise)",
      "GET /llms-full.txt": "Agent-optimized product catalog (llms.txt format, full details with specs)",
...
app.get("/", (c) => {
  ...
  return c.json({
    ...
    dataSource: c.env.SHOPIFY_STORE_DOMAIN ? `shopify (${c.env.SHOPIFY_STORE_DOMAIN})` : "sample catalog",
    endpoints: {
      "/llms.txt": "Agent-optimized product catalog (concise)",
      "/llms-full.txt": "Agent-optimized product catalog (full details with specs)",
