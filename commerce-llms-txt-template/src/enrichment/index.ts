...
${specsText}`;
  ...
  try {
    ...
    return {
      ...product,
      agentSummary: parsed.agentSummary ?? fallbackEnrichment(product).agentSummary,
...
export function fallbackEnrichment(product: RawProduct): EnrichedProduct {
  ...
  return {
    ...
    highlights: Object.entries(product.specs).slice(0, 3).map(([k, v]) => `${k}: ${v}`),
 ...
 */
const FALLBACK_ENRICHMENTS: Record<string, Pick<EnrichedProduct, "agentSummary" | "useCaseTags" | "highlights" | "bestFor">> = {
